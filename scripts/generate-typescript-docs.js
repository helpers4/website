...
function generateNamingConflictsPage(nameConflicts) {
  ...
  const tableRows = sortedNames.map(name => {
    ...
    const catLinks = cats.map(c => `[\`${c}\`](../categories/${c}/${name.toLowerCase()}/)`).join(', ');
...
function syncVersion() {
  ...
  for (const page of pages) {
    ...
    // 5. pnpm version pin comment: # pnpm add helpers4@X.Y.Z (unified package, since v3.1.0)
    patched = patched.replace(
      new RegExp(`(# pnpm add helpers4@)${SEM_VER_RE.source}`, 'g'),
      `$1${LIBRARY_VERSION}`
    );
...
/**
 * Publishes the pre-built `@helpers4/all` llms.txt (full signatures/params/examples for every
 * function, generated by scripts/build/build-llms-txt.ts in the typescript repo) as a static
 * file under this DOCS_TARGET's own site path — e.g. /typescript/llms-full.txt for the "latest"
 * slot, /typescript/next/llms-full.txt for the next-preview slot. This is the deep-dive TS
 * reference the site-level /llms.txt (public/llms.txt, hand-authored, not generated) links to
 * instead of duplicating its content.
 */
function copyLlmsFullText() {
  const source = path.join(buildPath, 'all', 'llms.txt');
  if (!fs.existsSync(source)) {
    console.warn(`  ⚠️  ${source} not found — skipping llms-full.txt (run \`pnpm build\` in typescript first)`);
    return;
  }
  const destDir = path.join(rootDir, 'public', DOCS_TARGET);
  fs.mkdirSync(destDir, { recursive: true });
  fs.copyFileSync(source, path.join(destDir, 'llms-full.txt'));
  console.log(`  ✓ published llms-full.txt → public/${DOCS_TARGET}/llms-full.txt`);
}
...
function archiveStableIfMajorBump(docsTarget, libraryVersion) {
  ...
  // Archive this major's llms-full.txt snapshot too, before the "latest" copy gets
  // overwritten by the new major version's build (same reasoning as ARCHIVABLE_ENTRIES above).
  const llmsFullSource = path.join(rootDir, 'public', product, 'llms-full.txt');
  if (fs.existsSync(llmsFullSource)) {
    const llmsFullArchiveDir = path.join(rootDir, 'public', archiveSlug);
    fs.mkdirSync(llmsFullArchiveDir, { recursive: true });
    fs.copyFileSync(llmsFullSource, path.join(llmsFullArchiveDir, 'llms-full.txt'));
  }
