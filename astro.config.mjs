 ...
 *
 * The AI & LLM Support page (which links DeepWiki) only exists under `typescript/reference/` for
 * the `latest` slot — DeepWiki only indexes `main`, and both `next` (unreleased preview) and
 * archived `vN` slots describe something other than what's currently on `main`, so the page
 * (and its autogenerated sidebar entry) isn't authored for those slots at all.
 */
...
// `hidden: true` entries still need a real topic — starlight-sidebar-topics requires every
// content page to resolve to some topic, so the "typescript/next" pages would break the build
// if their topic didn't exist at all. Hiding happens purely at the display layer instead: see
// TopicsSwitcher.astro (switcher row) and lib/versions.ts (version dropdown), both of which
// filter on this same `hidden` flag from versions.json.
